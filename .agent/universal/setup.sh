...
# Default testContextClass if not found — agent will use inline AemContext when this is blank
if [ -z "$TEST_CONTEXT_CLASS" ]; then
    TEST_CONTEXT_CLASS="AppAemContext"
    warn "No AemContext/TestContext class found — defaulting testContextClass to 'AppAemContext'. If your project uses a different test context class, update java.testContextClass in project.yaml."
fi

echo "   Package:    $JAVA_PACKAGE"
echo "   Test ctx:   ${TEST_CONTEXT_CLASS}"
...
if [ ! -d "$PROJECT_ROOT/$MODULE_UI_APPS/src/main/content/jcr_root${APPS_ROOT}/components/container" ]; then
    # Try common alternative container component names
    _found_container=""
    for _cname in "container" "responsivegrid" "parsys" "iparys" "iparsys"; do
        if [ -d "$PROJECT_ROOT/$MODULE_UI_APPS/src/main/content/jcr_root${APPS_ROOT}/components/${_cname}" ]; then
            CONTAINER_RESOURCE_TYPE="${APP_NAME}/components/${_cname}"
            _found_container="yes"
            break
        fi
    done
    if [ -z "$_found_container" ]; then
        # Fall back to AEM Core Components responsive grid (safe default for all AEM versions)
        CONTAINER_RESOURCE_TYPE="wcm/wcm/foundation/components/responsivegrid"
    fi
fi
...
# projectRoot: Absolute path to your AEM project root on this machine.
# This is the directory that contains pom.xml and the .agent/ folder.
# Adjust if you move the project or run on a different machine.
